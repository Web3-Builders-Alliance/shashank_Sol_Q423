...
import { ASSOCIATED_TOKEN_PROGRAM_ID as associatedTokenProgram, TOKEN_PROGRAM_ID as tokenProgram, createMint, createAccount, mintTo, getAssociatedTokenAddress, getAssociatedTokenAddressSync, getMinimumBalanceForRentExemptMint, MINT_SIZE, TOKEN_PROGRAM_ID, createInitializeAccount2Instruction, createInitializeMint2Instruction, createAssociatedTokenAccountIdempotentInstruction, createMintToInstruction, ASSOCIATED_TOKEN_PROGRAM_ID } from "@solana/spl-token"
...
describe("anchor-escrow", () => {
  ...
  // let makerA: PublicKey;
  // let takerB: PublicKey;

  it("Airdrop", async () => {
    let airdropIx=await Promise.all([maker, taker].map(async (k) => {
      return await anchor.getProvider().connection.requestAirdrop(k.publicKey, 10*LAMPORTS_PER_SOL)
      .then(confirm)
    }))
  ...
  await provider.sendAndConfirm(
    tx,[mintA,mintB,maker,taker]).then(log)
